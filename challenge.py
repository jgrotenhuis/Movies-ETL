...
file_dir = "/Users/grotenhuis/Desktop/Bootcamp/Movies-ETL/"

wiki_movies_raw = json.load(f"{file_dir}/wikipedia.movies.json")
kaggle_metadata = pd.read_csv(f"{file_dir}movies_metadata.csv", low_memory=False)        
ratings = pd.read_csv(f"{file_dir}ratings.csv")

def auto_ETL(wiki_movies, kaggle_data, movie_ratings):
        ...
        def clean_movie(movie):
            ...
            movie = dict(movie)
            # Create a dictionary for alternative titles.
            alt_titles = {}
            # Loop through every alternative title
        ...
        try:
            kaggle_data[~kaggle_data["adult"].isin(["True","False"])]
        except:
            print("Kaggle data is not in true/false")
        ...
        try:
            ...
            print("Error because cannot convert budget to the type int")
        ...
        rating_counts = movie_ratings.groupby(['movieId','rating'], as_index=False).count()
        rating_counts = movie_ratings.groupby(['movieId','rating'], as_index=False).count() \
        ...
        data = pd.read_csv(f'{file_dir}ratings.csv', chunksize=1000000)
        #print(f'importing rows {rows_imported} to {rows_imported + len(data)}...', end='')
        data.to_sql(name='movie_ratings', con=engine, if_exists='append')
